...
def getIdfromindex(index):
    return movies[movies.index == index]["id"].values[0]

def fetch_poster(movie_title):
    mov = get_index_from_title(movie_title)
    movId = getIdfromindex(mov) 
    url = "https://api.themoviedb.org/3/movie/19995?api_key=ac000c2664c50051813e0ab7d6bba0ac&language=en-US".format(movId)
...
def getMCRSReccommend(movie_title):
    ...
    #Define the weights to be used for the topsis should be equal to 1 
